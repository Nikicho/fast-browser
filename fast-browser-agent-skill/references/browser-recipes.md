...
如果任务是前端功能测试，低层配方主要用于失败后的定位与补证，不是人类日常主入口。
...
- 如果任务本质是回归验证，先确认是否已有 `case / flow`，不要一上来进入 DOM 探索
...
- `console / network` 主要在回归失败后使用，不应替代 `case / flow`
- 给人类汇报时，先说失败发生在哪个路径或断言，再补日志和请求证据
