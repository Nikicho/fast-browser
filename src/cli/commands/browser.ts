...
export function registerBrowserCommands(program: Command, deps: CliDependencies): void {
  ...
  program.command("eval").argument("<expression>").option("--json").action(async (expression: string, options) => {

    printOutput(await deps.router.evalExpression(expression), options.json);
